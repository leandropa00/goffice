        ...
        return datatables()
            ...
            ->addColumn('role', function ($row) use ($roles) {
                ...
                if ($row->id != 1) {
                    ...
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn ' . (($row->roleId == 1) ? 'btn-danger' : 'btn-info') . ' dropdown-toggle waves-effect waves-light btn-xs" type="button">' . (($row->roleId != 1) ? $row->current_role_name : __('app.' . $row->roleName)) . '</button>';
