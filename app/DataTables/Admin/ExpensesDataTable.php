...
class ExpensesDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->editColumn('status', function ($row) {
                ...
                if ($row->status == 'pending') {
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn dropdown-toggle waves-effect waves-light btn-xs btn-warning" type="button">' . strtoupper($row->status) . '</button>';
                } else if ($row->status == 'approved') {
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn dropdown-toggle waves-effect waves-light btn-xs btn-success" type="button">' . strtoupper($row->status) . '</button>';
                } else {
                    $status .= '<button aria-expanded="true" data-toggle="dropdown" class="btn dropdown-toggle waves-effect waves-light btn-xs btn-danger" type="button">' . strtoupper($row->status) . '</button>';
                }
    ...
    public function query(Expense $model)
    {
        ...
        $model = $model->with('currency')->select('expenses.id', 'expenses.item_name', 'expenses.user_id', 'expenses.price', 'users.name', 'expenses.purchase_date', 'expenses.currency_id', 'currencies.currency_symbol', 'expenses.status', 'expenses.purchase_from')
