...
class CreateSocialAuthSettingsTable extends Migration
{
    ...
    public function up()
    {
        ...
        $decodedSettings = json_decode($menuSettings->getOriginal('setting_menu'),true);
        $decodedSettings[] = ['id' => $menuData->id];
        $settings = json_encode($decodedSettings);

        $defaultDecodedSettings = json_decode($menuSettings->getOriginal('default_setting_menu'),true);
